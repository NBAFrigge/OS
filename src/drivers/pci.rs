...
impl PciAddress {
    ...
    pub fn set_bus(&mut self, bus: u32) {
        self.config_address =
            (self.config_address & !(0xFF << 16)) | ((bus & 0xFF) << 16);
    }

    pub fn set_device(&mut self, device: u32) {
        self.config_address =
            (self.config_address & !(0x1F << 11)) | ((device & 0x1F) << 11);
    }

    pub fn set_function(&mut self, function: u32) {
        self.config_address =
            (self.config_address & !(0x07 << 8)) | ((function & 0x07) << 8);
    }
    ...
    pub fn read_id_register(&mut self) {
        let addr =
            PciAddress::new(self.bus(), self.device(), self.function(), 0);
    ...
    pub fn read_class_register(&mut self) {
        let addr =
            PciAddress::new(self.bus(), self.device(), self.function(), 8);
        let mut addr_port = Port::<u32>::new(PCI_CONFIG_ADDR);
        let mut data_port = Port::<u32>::new(PCI_CONFIG_DATA);
        unsafe {
            addr_port.write(addr.raw());
        ...
        for i in 0..6 {
            ...
            unsafe {
                ...
                if (original_bar & PCI_BAR_TYPE_IO) != 0 {
                    ...
                    let size = (!(response_mask & PCI_BAR_IO_ADDR_MASK))
                        .wrapping_add(1);
                    ...
                    let is_prefetchable =
                        (original_bar & PCI_BAR_PREFETCHABLE) != 0;
                    let is_64bit = (original_bar & PCI_BAR_MEM_TYPE_MASK)
                        == PCI_BAR_MEM_TYPE_64;

                    if !is_64bit {
                        let address =
                            (original_bar & PCI_BAR_MEM_ADDR_MASK) as u64;
                        let size = (!(response_mask & PCI_BAR_MEM_ADDR_MASK))
                            .wrapping_add(1);
                        ...
                        let address = ((original_bar & PCI_BAR_MEM_ADDR_MASK)
                            as u64)
                            | ((original_high as u64) << 32);

                        let full_mask = ((response_mask & PCI_BAR_MEM_ADDR_MASK)
                            as u64)
    ...
    pub fn enable_bus_mastering(&mut self) {
        let addr =
            PciAddress::new(self.bus(), self.device(), self.function(), 0x04);
        let mut addr_port = Port::<u32>::new(PCI_CONFIG_ADDR);
        let mut data_port = Port::<u32>::new(PCI_CONFIG_DATA);
        unsafe {
            addr_port.write(addr.raw());
